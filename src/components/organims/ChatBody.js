  ...
  ({
    ...
    const generateChatDataWithSeparators = useCallback((messages = []) => {
      ...
      for (const msg of sortedMessages) {
        ...
        if (msg?.conversationEnded) {
          result.push({
            ...
            content: {
              ...
              icon: (
                <Text style={{fontSize: fontStyle.bodyMediumMedium12.fontSize}}>
                  ✅
                </Text>
              ),
            },
        ...
        if (
          (msg.status === socketConstants.failed ||
            msg.status === socketConstants.pending) &&
          !isHistoricalMessage
        ) {
          result.push({
    ...
    return (
      <FlatList
        ...
        onEndReached={() =>
          hasMore &&
          !historyLoading &&
          reconfigApiResponse?.userInfo?.agentId &&
          loadChatHistory(
            reconfigApiResponse?.userInfo?.agentId,
            page,
            5,
